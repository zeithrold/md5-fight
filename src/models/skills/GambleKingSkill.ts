...
export default class GambleKingSkill extends Skill {
  ...
  static readonly displayName = '777';

  static readonly description = `攻击者在攻击阶段有40%的概率触发。
  丢一枚硬币。若结果为正面，本轮攻击被攻击者的防御无效；若结果为反面，攻击者在攻击前生命值减少最大生命值的30%`;

  static readonly quote = '敢问您的17张牌能不能秒了我。';
  ...
  effect() {
    ...
    if (isAffects) {
      this.api.addLog({
        message: `<b>${this.owner}</b>的技能<b>${GambleKingSkill.displayName}</b>发动！开始丢一枚硬币。`,
        bgColor: 'primary',
      });
