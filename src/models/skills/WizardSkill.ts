...
export default class WizardSkill extends Skill {
  ...
  static readonly displayName = '巫师';

  static readonly description = '对手有15%的概率获得5回合的"毒药"效果，每回合前将对手生命值扣除其生命值最大值的10%，若对手已有同效果，则仅刷新次数，不叠加。';

  static readonly quote = '可能只是给对手喂了芙蓉秘制小汉堡。';

  effect() {
    const isAffect = Math.floor(Math.random() * 100) <= 15;
    if (isAffect) {
      this.api.addLog({
        message: `<b>${this.owner}</b>的技能<b>${WizardSkill.displayName}</b>发动！向被攻击者施加5回合的"<b>毒药</b>"效果。`,
        bgColor: 'primary',
      });
